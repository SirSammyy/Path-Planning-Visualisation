...
def dijkstraSearch (img, mapToPath, start, goal, squareHeight, squareWidth):
    ...
    checkedNodes = [[0 for y in range(totalAmountOfSquaresHeight)] for x in range(totalAmountOfSquaresWidth)]
    checkedNodesShow = []
    ...
    while(totalOfOpenSet != 0):
        ...
        if (cy == gy and cx == gx):
            ...
            checkedNodesShow.append([gy,gx])
            finalPath = reconstruct_path(cameFrom, current, start)
            return checkedNodesShow, finalPath
        
        #Adds cy and cx to checkedNodes as a tuple
        checkedNodes[cy][cx] = 1
        checkedNodesShow.append([cy,cx])
        ...
        for i in range(0, totalNeighbours):
            ...
            if ((mapToPath[ny][nx]) == 1):
                ...
                checkedNodes[ny][nx] = 1
                checkedNodesShow.append([ny,nx])
                continue
