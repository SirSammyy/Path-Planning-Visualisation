...
def depthFirstSearch (img, mapToPath, start, goal):
    ...
    while(totalOfOpenSet != 0):
        #Finds the total and takes 1 off as lists and arrays start at 0!
        openSetMax = len(openSet)- 1     
        #Sets current to be the first item in openSet
        current = openSet[openSetMax]
        #Removes the first item from openSet, this is currently current
        del openSet[openSetMax]
        ...
        neighbours, totalNeighbours = findNeighbours(cy,cx)
